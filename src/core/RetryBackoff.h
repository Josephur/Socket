...
// Staggered retry schedule: waits 3, 5, 10, then 15s between attempts,
// holding at 15s thereafter until reset(). Used to keep the OFFLINE state
// from hammering the WiFi stack or the provisioning endpoint on every
// loop() tick.
class RetryBackoff {
 public:
  RetryBackoff() { reset(); }
 ...
 private:
  static constexpr uint32_t kIntervalsSec[] = {3, 5, 10, 15};
  static constexpr size_t kNumIntervals = 4;
